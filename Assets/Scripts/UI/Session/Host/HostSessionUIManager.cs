...
{
    public class HostSessionUIManager : MonoBehaviour
    {
        [Header("Dependencies")]
        ...
        private void Start()
        {
            ...
            {
                _sessionManager = FindObjectOfType<Firebase.HostSessionManager>();
        ...
        {
            string url = $"https://jungame3757.github.io/WhatQuizClient/?code={code}";
        ...
        private void CopySessionCodeToClipboard()
        {
            if (!string.IsNullOrEmpty(_sessionCode))
            {
                string url = $"https://jungame3757.github.io/WhatQuizClient/?code={_sessionCode}";
        ...
        private void KickPlayer(string playerId)
        {
            if (_isLocalPlayerHost && !string.IsNullOrEmpty(playerId))
            {
                ...
                {
                    Debug.LogError("세션 매니저를 찾을 수 없습니다!");
                    UI.UISystem.DebugManager.Instance.ShowMassage("오류", "세션 매니저를 찾을 수 없습니다.", null, null);
                }
            }
        }
        ...
        private string GetLocalPlayerId()
        {
            ...
            if (Firebase.HostAuthManager.Instance != null)
            {
                try
                {
                    // Firebase.AuthManager에서 적절한 메서드 호출
                    string userId = HostAuthManager.Instance.GetIdToken(); // 임시 방법: 토큰을 ID로 사용
